...
struct page * check_address(uaddr);
void halt(void);
...
struct page * check_address(void *addr) {
	...
	/*
	else if(pml4_get_page(cur->pml4, addr) == NULL)
	{
		printf("Check address fault at - %p\n", addr);
	}
	*/
...
//beffer 의 끝까지 address를 체크해주어야한다.
...
void *mmap (void *addr, size_t length, int writable, int fd, off_t offset){
	//void *page_addr = addr;
	if(length == 0 || addr == 0 || offset > PGSIZE)// || pg_ofs(addr) != 0)
		return NULL;
	...
	if (pg_round_down(addr) != addr || is_kernel_vaddr(addr) || addr == NULL || (long long)length <= 0)
		return NULL;
	// page_round_down, kernel address 인지 확인 
